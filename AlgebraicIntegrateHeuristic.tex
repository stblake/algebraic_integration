...
We will be describing a seemingly new method for computing pseudo-elliptic integrals. A 
pseudo-elliptic integral is of the form 
...
The \textit{derivative divides} method is a substitution method which finds all composite 
...
Then the substitution $u=x+1/x$, $du=1-1\left/x^2\right.dx$ yields the integral
...
bottlenecks which result in long computations. So for algebraic functions a domain-specific 
...
$\int\frac{1-1\left/x^2\right.}{(x+1/x)\sqrt{(x+1/x)^2-2}} \, dx$ in order to discover the rational 
...
\end{equation}
where $a_0$, $a_1$, $a_2$, $u(x)$ are known and $a(u)$, $b(u)$ are unknown. The degree bound estimate of  
...
Our test suite is 190 integrals which can be found on github\cite{test_suite_github}. 
...
In regards to this table: The median time excludes integrals which timed-out. For 
...
Maple fairs much better when it uses the algebraic case of the Risch-Trager-Bronstein 
